...
not its surface. Swagbound's distinct lane stays: provenance, the "public version,"
...
|---|---|---|---|
| 1 | Meteor crash overnight | At ~4am Bosch's phone mints "Swag is eternal" signed as him; the Route Brief "opens from outside the house" while he sleeps. The thing that "arrived overnight" = the **public version of Bosch**. | new (intro beat) |
| 2 | Pokey knocks, wakes hero | **MiFella** wakes Bosch: "you're trending before breakfast." Companion who pulls him out + tags toward the trouble (and is quietly public-version-aligned). **Biscuit (the dog)** is the household pet. | `interior:bosch-bedroom-v0`, `interior:home-hall-v0` (reattribute waker → MiFella) |
...
| 4 | Buzz Buzz exposition | The **Bosch Terminal** ("the route is not neutral; a public version is moving through town ahead of your body…") — an oracle from *ahead*, not the future. Frames the act + names **Milady** as the engine. | `dialogue:archive-console-link` |
...
| 7 | Morning: roadblock + Sharks gate the town | The **civic route**: town services classifying the copy; the **Public Version Clique** controls the route. The **Route Clerk** gives the required order (witness → proof → refusal → parks → gate). | new (clerk) + `interior:bosch-main-room-v0` |
| 8 | Frank → Frankystein Mk2 | The **Public Version Clique** confrontation; its leader = Frank analog; the escalated "Mk2" form = the clique's pre-signed clean Bosch. You **refuse the clean version in public**. Prep first at **Sal** (smudged proof). | `interior:corner-shop-v0` (Sal), new (clique) |
...
| 10 | Giant Step → Titanic Ant (first Sanctuary) | The first **contested park / threshold** = first "Sanctuary"; its boss = the **Malady** (the public-version mask). Defeating it = first recognition milestone; the mask peels to Bosch's face, "CONTRACT OWNER: Milady." | new (park + Malady) |
| 11 | Leave town north | **Refuse Processing** — keep the *damaged* Public Version Card, refuse the stamp; leave Signal Town for the next district. | new (compliance window) |

## Cast / system correspondence
| EarthBound | Swagbound | Note |
|---|---|---|
| Ness (hero) | **Bosch** | Accidental, human, not pre-chosen. |
| Pokey (next-door, wakes you; later villain) | **MiFella** | The companion who pulls Bosch out; quietly public-version/Milady-aligned — carries the Pokey heel turn. |
...
| Picky (found at the site) | the **public-version trace** / first **witness** account (Bonkle) | What you "find" at the anomaly. |
...
| Sharks / **Frank** | **Public Version Clique** (+ leader) | Route-gating antagonist gang. |
...
| Mani-Mani statue | **Public Version Card** | User's declared equivalence: desire → item → credential → enemy artifact. |
...
- **Companion cast:** **MiFella** is the Pokey equivalent (companion → public-version
  heel); **Biscuit is the dog** (EB's King); **Guardian** is the Mom (charm-giver). The
  Pokey betrayal runs through MiFella + the public-version/Milady thread.
